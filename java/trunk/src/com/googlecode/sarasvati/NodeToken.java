...
public interface NodeToken extends Token
{
  ...
  /**
   * A NodeToken is evaluated by the {@link Node#guard(BaseEngine, NodeToken)}
   * method. recordGuardAction will be called by the engine to
   * record the result of the guard.
   *
   * @param engine The {@link BaseEngine} being used to execute the associated {@link Process}.
   * @param action The {@link GuardAction} taken with this NodeToken.
   * @see Node#guard(BaseEngine, NodeToken)
   */
  void recordGuardAction (BaseEngine engine, GuardAction action);
